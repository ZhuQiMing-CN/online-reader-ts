...
export default class Bookshelf extends Vue {
    ...
    // 移除书架
    private delBooks (val: any) {
        let delbook = this.bookShelfs.findIndex((item: any) => item.id === val.id);
        this.bookShelfs.splice(delbook, 1);
        this.$store.commit('setBookShelf', this.bookShelfs);
    }
